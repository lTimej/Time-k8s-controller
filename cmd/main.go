...
import (
	...
	"Time-k8s-controller/internal/controller"
	"Time-k8s-controller/middleware"
	"Time-k8s-controller/pb"
	"Time-k8s-controller/service"
	"Time-k8s-controller/tools/signal"
...
func main() {
	...
	if err = controller.NewPodReconciler(mgr.GetClient(), mgr.GetScheme(), manager).SetupWithManager(mgr); err != nil {
...
func StartGrpcServer(client client.Client, manager *statussync.StatusInformer) *grpc.Server {
	...
	pb.RegisterServiceServer(server, service.NewSpaceService(client, manager))
